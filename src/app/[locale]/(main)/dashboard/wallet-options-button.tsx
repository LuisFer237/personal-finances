  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(data.error || t("wallet.deleteFailed", "Failed to delete wallet"));
        return;
      }

      toast.success(t("wallet.deleted", "Wallet deleted!"));
      router.refresh();
    } catch (error) {
      toast.error(t("wallet.deleteError", "An error occurred while deleting the wallet"));
    } finally {
      setIsDeleting(false);
  ...
  return (
    ...
    <div onClick={(e) => e.stopPropagation()}>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>{t("wallet.edit", "Edit Wallet")}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle>{t("wallet.confirmDeleteTitle", "Are you absolutely sure?")}</DialogTitle>
            <DialogDescription>
              {t("wallet.confirmDeleteDescription", "This action cannot be undone. This will permanently delete your wallet")} <strong>{wallet.name}</strong> {t("wallet.confirmDeleteDescriptionTail", "and all associated data.")}
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end gap-3 mt-4">
            ...
            <Button
              ...
              disabled={isDeleting}
            >
              {isDeleting ? t("common.deleting", "Deleting...") : t("wallet.confirmDelete", "Yes, delete wallet")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
